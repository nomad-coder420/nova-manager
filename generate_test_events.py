...
from nova_manager.components.metrics.events_controller import EventsController, TrackEvent
from nova_manager.components.users.models import Users
from nova_manager.database.session import SessionLocal, get_db
...
from nova_manager.components.user_experience import event_listeners as ux_event_listeners  # noqa: F401


class TestEventGenerator:
    def __init__(self, db: Session, organisation_id: str = "org123", app_id: str = "app123"):
        self.db = db
        ...
        self.user_pool = [f"user_{i+1:03d}" for i in range(20)]  # String user IDs like "user_001", "user_002", etc.
    ...
    def create_test_users(self) -> List[UUID]:
        ...
        for i, user_id in enumerate(self.user_pool):
            ...
            if existing_user:
                ...
                existing_count += 1
                print(f"✅ User {i+1} ({user_id}): Found existing user (pid: {existing_user.pid})")
            else:
                # User doesn't exist, create new one
                # Generate comprehensive user profile data
                signup_days_ago = random.randint(1, 365)
                signup_date = datetime.now(timezone.utc) - timedelta(days=signup_days_ago)
                
                user_profile = {
                    # Demographics
                    "age": random.randint(18, 65),
                    "gender": random.choice(["male", "female", "other", "prefer_not_to_say"]),
                    "country": random.choice(["US", "UK", "CA", "AU", "DE", "FR", "JP", "BR"]),
                    "city": random.choice(["New York", "London", "Toronto", "Sydney", "Berlin", "Paris", "Tokyo", "São Paulo"]),
                    "timezone": random.choice(["UTC-8", "UTC-5", "UTC", "UTC+1", "UTC+9"]),
                    
                    # Subscription & Business
                    "subscription_plan": random.choice(["free", "basic", "premium", "enterprise"]),
                    "subscription_status": random.choice(["active", "trial", "expired", "cancelled"]),
                    "account_type": random.choice(["personal", "business", "student", "non_profit"]),
                    "signup_date": signup_date.isoformat(),
                    "last_login_date": (datetime.now(timezone.utc) - timedelta(days=random.randint(0, 30))).isoformat(),
                    ...
                    "preferred_language": random.choice(["en", "es", "fr", "de", "pt", "ja"]),
                    ...
                    "app_version": random.choice(["1.0.0", "1.1.0", "1.2.0", "2.0.0", "2.1.0"]),
                    
                    # Acquisition & Attribution
                    "acquisition_channel": random.choice(["organic_search", "paid_search", "social_media", "email", "referral", "direct"]),
                    "referral_source": random.choice(["google", "facebook", "twitter", "linkedin", "friend", "advertisement"]),
                    "utm_campaign": random.choice(["summer_sale", "new_user", "retargeting", "brand_awareness"]),
                    "first_touch_channel": random.choice(["organic", "paid", "social", "email", "direct"]),
                    
                    # Segmentation
                    "user_segment": random.choice(["new_user", "active_user", "at_risk", "champion", "dormant"]),
                ...
                created_count += 1
                print(f"✅ User {i+1} ({user_id}): Created new user with {len(user_profile)} properties (pid: {user.pid})")
            
        # Commit all changes - track_user_profile will be called automatically via event listener for new users
        if created_count > 0:
            self.db.commit()
            print(f"📝 {created_count} new user profiles automatically tracked via SQLAlchemy event listener")
        
        print(f"📊 Summary: {existing_count} existing users found, {created_count} new users created")
        
        return user_pids

    def generate_historical_events(self, days: int = 7, events_per_day: int = 50) -> int:
        """Generate historical events over the specified number of days using bulk tracking"""
        total_events = 0
        
        print(f"🚀 Generating {days} days of historical events ({events_per_day} events/day)...")
            ...
            for user_id, events in user_events.items():
                if events:  # Only send if user has events for this day
                    try:
                        ...
                        print(f"❌ Error tracking {len(events)} events for user {user_id}: {e}")
            
            print(f"✅ Day -{day}: Generated {day_events} events across {len([u for u, e in user_events.items() if e])} users")
        ...
        for user_id, events in user_events.items():
            if events:  # Only send if user has events
                try:
                    ...
                    print(f"✅ User {str(user_id)[:8]}...: Sent {len(events)} events in bulk")
                    
                except Exception as e:
                    print(f"❌ User {str(user_id)[:8]}...: Failed to send {len(events)} events: {e}")
...
def main():
    ...
    try:
        ...
        generator.user_pool = user_pids  # Update user_pool with actual user PIDs for event tracking
        print(f"✅ Ready with {len(user_pids)} test users with 30+ profile properties each")
        ...
        print(f"🚀 Performance: Events sent in bulk batches per user")
        
        print(f"\n🧪 Test your metric builder with:")
        print(f"- Count metrics: 'user_signup', 'purchase_completed', 'page_view'")
        print(f"- Aggregation metrics: sum of 'amount', avg 'session_duration'")
        print(f"- Ratio metrics: purchase_completed / user_signup")
        print(f"- Group by event properties: 'device_type', 'payment_method', 'platform'")
        print(f"- Group by user profile: 'country', 'subscription_plan', 'user_segment'")
        print(f"- Filter by event properties: 'currency', 'ab_test_variant', 'product_category'")
        print(f"- Filter by user profile: 'age', 'country', 'subscription_plan', 'customer_tier'")
        print(f"\n📊 User Profile Properties Available for Filtering:")
        print(f"  Demographics: age, gender, country, city, timezone")
        print(f"  Subscription: subscription_plan, subscription_status, account_type")
        print(f"  Financial: total_spent, average_order_value, lifetime_value")
        print(f"  Engagement: total_sessions, engagement_score, last_active_days_ago")
        print(f"  Technical: device_type, operating_system, browser, app_version")
        print(f"  Marketing: acquisition_channel, utm_campaign, marketing_consent")
        print(f"  Segmentation: user_segment, customer_tier, is_premium, is_power_user")
            
    except Exception as e:
        traceback.print_exc()
        print(f"❌ Error: {e}")
        print("Make sure your database is set up and Nova Manager dependencies are installed")
        db.rollback()
        raise e
    finally:
        db.close()
